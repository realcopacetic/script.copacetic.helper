...
from resources.lib.shared.utilities import (ADDON, condition, infolabel,
                                            json_call, log, return_label,
                                            set_plugincontent, split,
                                            split_random, url_encode,
...
class DataHandler:
    ...
    def _resumepoint(self):
        ...
        if "set" in self.dbtype:
            if self._wait_for_set_match():
                ...
                return ((total and watched / total or 0) * 100), (total - watched) # https://stackoverflow.com/a/68118106/21112145 to avoid ZeroDivisionError
        return 0, unwatched
    ...
    def _wait_for_set_match(self):
        ...
        while time.time() < timeout:
            if(
                condition("String.IsEqual(ListItem.DBID,Container(3100).ListItem.SetID)")
    ...
    def _multiart(self):
        if not (self._wait_for_art() and (art_type := infolabel("Control.GetLabel(6400)"))):
            return {}
        return {
            f"multiart{pos if pos else ''}": art
            for pos in range(16)
            if (art := infolabel(f"{self.listitem}.Art({art_type}{pos if pos else ''})"))
        }
